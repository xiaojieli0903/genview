...
class CIFAR10(BaseDataset):
    ...
    def load_data_list(self):
        ...
        data_list = []
        img_dir = os.path.join(self.data_root, self.base_folder, self.split + '_imgs')
        data_file = open(os.path.join(self.data_root, self.base_folder, self.split + '.txt'), 'w')
        if not os.path.exists(img_dir): os.makedirs(img_dir)
        for img, gt_label, filename in zip(imgs, gt_labels, filenames):
            info = {'img': img, 'gt_label': int(gt_label)}
            out_path = os.path.join(img_dir, filename)
            Image.fromarray(img).save(out_path)
            data_file.write(f'{out_path} {gt_label}\n')
