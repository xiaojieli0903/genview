...
class LoadImageFromFileDiffusion(BaseTransform):
    ...
    def __init__(self,
                 ...
                 train_scales: Optional[list] = None) -> None:
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if 'gen_img_path' not in results:
            return results

        filename = results['gen_img_path']
        if isinstance(filename, list):
            if self.train_scales is None:
                chosen_idx = np.random.randint(len(filename))
                filename = filename[chosen_idx]
                if filename.find('s-') < 0:
                    scale = 'default'
                else:
                    scale = filename.split('_s-')[1].split('.')[0]
                results['gen_img_path'] = filename
                results['gen_img_scale'] = scale
            else:
                chosen_scale = random.sample(self.train_scales, 1)[0]
                use_gen = False
                for gen_img in filename:
                    scale = gen_img.split('_s-')[1].split('.')[0]
                    if scale == chosen_scale or str(scale) == str(chosen_scale):
                        filename = gen_img
                        results['gen_img_path'] = gen_img
                        results['gen_img_scale'] = chosen_scale
                        use_gen = True
                        break
                if not use_gen:
                    raise RuntimeError(
                        f'gen_img_path exists in results.keys().'
                        f' But failed when combined with train_scales = {self.train_scales}'
                        f' parsed scale from {gen_img} is {scale}'
                    )
        try:
            if self.file_client_args is not None:
                file_client = fileio.FileClient.infer_client(
        ...
        if self.to_float32:
            img = img.astype(np.float32)
        results['gen_img'] = img
        results['gen_img_shape'] = img.shape[:2]
        results['gen_ori_shape'] = img.shape[:2]
        return results
